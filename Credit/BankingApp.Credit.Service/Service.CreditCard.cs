...
{
    public partial class SCredit(IServiceProvider _serviceProvider): ISCredit
    {
        public async Task<MessageContainer> CardRevenuePaymentSchedule(MessageContainer requestMessage)
        {
            ...
            {
                // OZZGURFIXcardFee = decimal.Parse(parList.Find(x => x.Code.Equals(ccList[i].Type))!.Detail1!);
        ...
        public async Task<MessageContainer> CardExpensePayment(MessageContainer requestMessage)
        {
            ...
            else
            {
                ...
                {
                    await proxy.AddNewTransaction(requestTH);
                }
        ...
        {
            ECreditCard eCreditCard = new ECreditCard(requestMessage.UnitOfWork);
            EAccount eAccount = new EAccount(requestMessage.UnitOfWork);    //OZZGUR FIX
